...
function verifyPassword() {
    ...
    let allString = all.join("").toLowerCase();

    matches = allString.matching_positions("q");

    for (var i = 0; i < matches.length; i++) {
        let currentMatch = matches[i];

        if ((allString.slice(currentMatch, currentMatch + 16)) == secret_password) {
            return true;
        }
    }
...
document.onkeydown = function (evt) {
    ...
    all.push(String.fromCharCode(charCode));
